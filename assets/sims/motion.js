...
window.SIMS['motion'] = (function(){
  ...
  // المضمار (أعلى) والرسم البياني (أسفل)
  var TKY=34, TKX0=38, TKX1=286;
  var GX0=46, GX1=286, GY0=64, GYB=216;      // منطقة الرسم
  function toAr(n){ return String(n).replace(/[0-9]/g,function(d){ return '٠١٢٣٤٥٦٧٨٩'[d]; }); }
  function fmt(x){ return toAr(Math.round(x)); }
  ...
  function mount(container){
    ...
    var t=0, d=0, trace=[[0,0]], running=!reduce, curState='';
    ...
    function drawTrack(){
      ...
      // المتحرّك (سيارة صغيرة)
      var cxp=TKX0+(Math.min(d,DMAX)/DMAX)*(TKX1-TKX0-22)+2, body=TKY-2;
      ctx.fillStyle=css('--subject','#0FA3AA');
      roundRect(cxp,body,22,10,3); ctx.fill();
      ctx.beginPath(); roundRect(cxp+3,body-5,11,6,2); ctx.fillStyle=css('--subject-deep','#0B7E86'); ctx.fill();
    ...
    function drawGraph(){
      ...
      // الخطّ
      if(trace.length>1){
        ctx.strokeStyle=css('--subject','#0FA3AA'); ctx.lineWidth=3; ctx.lineJoin='round'; ctx.lineCap='round';
        ctx.beginPath(); ctx.moveTo(px(trace[0][0]),py(trace[0][1]));
        for(var k=1;k<trace.length;k++) ctx.lineTo(px(trace[k][0]),py(trace[k][1]));
        ctx.stroke();
      }
      // النقطة الحالية
      ctx.fillStyle=css('--ember','#F2892E'); ctx.beginPath(); ctx.arc(px(t),py(d),4.5,0,6.2832); ctx.fill();
    }
    function roundRect(x,y,w,h,r){ ctx.beginPath(); ctx.moveTo(x+r,y); ctx.arcTo(x+w,y,x+w,y+h,r); ctx.arcTo(x+w,y+h,x,y+h,r); ctx.arcTo(x,y+h,x,y,r); ctx.arcTo(x,y,x+w,y,r); ctx.closePath(); }

    function draw(){ ctx.clearRect(0,0,W,H); drawGraph(); drawTrack(); }

    function refresh(){
      var sp=+sIn.value;
      q('.js-sval').textContent=toAr(sp);
      q('.js-eq').textContent=fmt(d)+' = '+toAr(sp)+' × '+fmt(t);
      elT.textContent=fmt(t); elD.textContent=fmt(d);
      var st = sp===0 ? 'rest' : 'move';
      if(st!==curState){
    ...
    function loop(){
      ...
      if(running && t<TMAX){ t+=DT; if(t>TMAX)t=TMAX; d+=(+sIn.value)*DT; trace.push([t,d]); }
      if(t>=TMAX && running){ running=false; q('.js-play').textContent='▶ تشغيل'; }
      refresh(); draw();
      requestAnimationFrame(loop);
    }

    function resetRun(){ t=0; d=0; trace=[[0,0]]; }
    sIn.addEventListener('input', refresh);
    q('.js-play').onclick=function(){
      if(t>=TMAX) resetRun();
      running=!running; if(t>=TMAX) running=true;
      this.textContent=running?'⏸ إيقاف':'▶ تشغيل';
    };
    q('.js-reset').onclick=function(){ resetRun(); running=true; q('.js-play').textContent='⏸ إيقاف'; refresh(); draw(); };
